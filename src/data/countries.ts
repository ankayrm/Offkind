...
export const DEFAULT_PHONE_COUNTRY = "GR";

const RAW: [iso: string, name: string, dial: string][] = [
  ["AF", "Afghanistan", "93"],
  ["AL", "Albania", "355"],
  ["DZ", "Algeria", "213"],
  ["AD", "Andorra", "376"],
...
export const phoneCountries: PhoneCountry[] = RAW.map(([iso, name, dial]) => ({
  iso,
  name,
  dial,
})).sort((a, b) => {
...
export function getPhoneCountry(iso: string | null | undefined): PhoneCountry {
  return byIso.get(iso ?? "") ?? byIso.get(DEFAULT_PHONE_COUNTRY)!;
}

export function isPhoneCountry(iso: string | null | undefined): iso is string {
  return Boolean(iso && byIso.has(iso));
}
...
export function nationalPhoneDigits(phone: string, iso: string): string {
...
export function formatInternationalPhone(phone: string, iso: string): string {
  const country = getPhoneCountry(iso);
